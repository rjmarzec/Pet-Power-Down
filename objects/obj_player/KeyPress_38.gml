// Player's northward/up movement
...
if(global.player_turn_counter > 0)
{
	...
	y_distance_left_to_move = global.movespeed_of_player*(global.player_step_direction_counters[1]-global.player_step_direction_counters[0])
	
	// Allow the movement if it won't put the player in a wall
	if(tilemap_get_at_pixel(layer_tilemap_get_id("Walls"), x + x_distance_left_to_move, y + y_distance_left_to_move - 32) == 0) 
	{
		// Add the number of steps they need to take and subtract a movement
		global.player_step_direction_counters[0] += global.step_amount_of_player;
		global.player_turn_counter -= 1;
	}
	// If the player will hit a wall, play a thud sound
	else
	{
		audio_play_sound_at(bump, x, y - global.tile_size/2, 0, 1, 32, 1, false, 0);
	}
}
